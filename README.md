...
Slightly modify from https://github.com/gilbertw1/slack-scala-client
to work well behind proxy.
...
```scala
import slack.api.MySlackApiClient          // Async
import slack.api.MyBlockingSlackApiClient  // Blocking
```
...
val proxyPort = 80
val client = MySlackApiClient(token,proxyHost,proxyPort)
```
...
```scala
val client = MyBlockingSlackApiClient(token,proxyHost,proxyPort)  // Default timeout of 5 seconds
...
val proxyHost = "proxy.example.com"
val proxyPort = 80
val client = SlackRtmClient(token,proxyHost,proxyPort)
```
...
val client = MySlackRtmClient(token,proxyHost,proxyPort)
