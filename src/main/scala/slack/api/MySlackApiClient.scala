...
/**
  * Created by hungdv on 13/08/2017.
  */
import java.io.File
...
object MySlackApiClient {
  ...
  def apply(token: String,proxyHost: String, proxyPort: Int): MySlackApiClient = {
    new MySlackApiClient(token,proxyHost: String, proxyPort: Int)
  }
  ...
  private def makeApiRequest(request: HttpRequest)(implicit system: ActorSystem): Future[JsValue] = {
    ...
    val httpsProxyTransport = ClientTransport.httpsProxy(InetSocketAddress.createUnresolved("172.30.45.220", 80))
    val settings = ConnectionPoolSettings(system).withTransport(httpsProxyTransport)
    Http().singleRequest(request,settings = settings).flatMap {
...
import MySlackApiClient._

class MySlackApiClient(token: String,proxyHost: String, proxyPort: Int) {
